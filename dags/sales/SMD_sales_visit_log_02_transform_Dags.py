...
# 함수 import
from modules.transform.pipelines.sales.SMD_sales_visit_log_02_transform import (
...
with DAG(
    ...
    # master로 신규 마스터 파일 전처리
    preprocess_master_tmp = PythonOperator(
        task_id='preprocess_master_tmp',
        python_callable=preprocess_master_tmp_df,
        op_kwargs={
            'input_xcom_key': 'visit_sales_log_master_path', # 마스터 파일 경로
            'output_xcom_key': 'preprocessed_master_tmp_path', # 전처리된 마스터 파일 경로
        }
    )
    ...
    join_master_flow = PythonOperator(
        ...
        op_kwargs={
            ...
            'right_task': {'task_id': 'preprocess_master_tmp', 'xcom_key': 'preprocessed_master_tmp_path'},
            'on': ['post_date', 'author'],
            'how': 'left',
    ...
    move_files_task = PythonOperator(
        ...
        op_kwargs={
            ...
            'source_dir': str(DOWN_DIR / '업로드_temp'),

...
dag_id = "DB_OKPOS_Sales_Launch_Only_Dags"
...
def resolve_launch_sale_dates(**context) -> str:
    today = pendulum.now("Asia/Seoul").format("YYYY-MM-DD")
    context["ti"].xcom_push(key="sale_dates", value=[today])
    logger.info("Launch 실행 날짜: %s", today)
    return f"Launch 날짜 결정 완료: {today}"


def skip_launch_global_task(**context) -> str:
    return "송파삼전점 Launch 범위 밖 전역 정리/점검 스킵"
...
def trigger_unified_sales_after_okpos(**context) -> str:
    ...
    try:
        trigger_dag(
            dag_id="DB_UnifiedSales_Launch_Dags",
            run_id=run_id,
            conf=trigger_conf,
        )
    except DagRunAlreadyExists:
        logger.info("DB_UnifiedSales_Launch_Dags 이미 트리거됨: %s", run_id)
        return f"DB_UnifiedSales_Launch_Dags 이미 트리거됨: {run_id}"

    logger.info("DB_UnifiedSales_Launch_Dags 트리거 완료: %s | conf=%s", run_id, trigger_conf)
    return f"DB_UnifiedSales_Launch_Dags 트리거 완료: {run_id}"
...
with DAG(
    ...
    t1 = PythonOperator(
        ...
        python_callable=resolve_launch_sale_dates,
    )

    t1b = PythonOperator(
        task_id="report_missing_daily",
        python_callable=skip_launch_global_task,
    )
    ...
    t6a = PythonOperator(
        ...
        python_callable=_reload_and_call,
        op_kwargs={"fn_name": "reconcile_today_vs_receipt", "target_stores": TARGET_STORES},
        retries=0,
        retry_delay=timedelta(minutes=3),
    )
    ...
    t6b = PythonOperator(
        ...
        python_callable=_reload_and_call,
        op_kwargs={"fn_name": "validate_today_vs_receipt", "target_stores": TARGET_STORES},
    )

    t6c = PythonOperator(
        task_id="reconcile_against_daily_summary",
        python_callable=_reload_and_call,
        op_kwargs={"fn_name": "reconcile_against_daily_summary", "target_stores": TARGET_STORES},
        retries=0,
    )

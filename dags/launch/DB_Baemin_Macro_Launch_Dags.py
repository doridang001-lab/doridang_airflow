...
def precheck_manual_baemin_orders(**context) -> str:
    dag_run = context.get("dag_run")
    conf = (getattr(dag_run, "conf", None) or {}) if dag_run else {}
    target_date = conf.get("target_date") or pendulum.now(KST).format("YYYY-MM-DD")
...
def collect_all(**context) -> str:
    ...
    if conf.get("target_date"):
        ...
        dates = [pendulum.now(KST).format("YYYY-MM-DD")]

"""
네이버 스마트플레이스 정책 수집 DAG

📋 처리 흐름:
1. Playwright로 공지 목록 수집 (GraphQL 캡처 + buildId 추출)
2. title+policy_date 중복 판정 및 신규 목록 생성
3. requests로 _next/data/{buildId}/notices/{seq}.json 본문 수집 및 LLM 분석
4. policy_type 표준 코드 매핑 (8종: 할인/광고/노출/수수료/정산/운영/기능변경/기타)
5. 정책행 누적 저장 및 중복제거(title+policy_date), 정렬(policy_date desc)

⚙️ 실행 시각: 매일 10:35 (KST, 정책 DAG 5분 간격 분산)
...
from modules.transform.utility.schedule import SMP_POLICY_NAVERPLACE_TIME
...
pipeline_module_name = "SMP_naver_place_policy_collect"
...
def task_collect_policy_rows(**context):
    """상세 본문 수집 (requests _next/data) 및 LLM(gpt-oss-20b) 기반 정책행 생성"""
...
with DAG(
    ...
    description="네이버 스마트플레이스 공지사항 정책 변경 수집 및 LLM 기반 분석",
    schedule=SMP_POLICY_NAVERPLACE_TIME,
    start_date=pendulum.datetime(2026, 4, 11, tz="Asia/Seoul"),
    catchup=False,
    max_active_runs=1,
    tags=["01_crawling", "naver_place", "policy", "daily"],
    default_args={
        "retries": 1,

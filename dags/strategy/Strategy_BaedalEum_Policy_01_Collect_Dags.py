...
📋 처리 흐름:
1. REST API 공지 목록 수집 (번호 있는 항목만, fixedContents 제외)
...
⚙️ 실행 시각: 매일 10:30 (KST, 정책 DAG 5분 간격 분산)
...
from modules.transform.utility.schedule import SMP_POLICY_BAEDALEUM_TIME
...
pipeline_module_name = "SMP_baedaleum_policy_collect"
...
def task_extract_notice_list(**context):
    """배달e음 REST API에서 공지 목록 파싱 (번호 있는 항목만)"""
...
# ============================================================
# Task 3: 정책 본문 파싱 및 LLM 분석
# ============================================================

def task_collect_policy_rows(**context):
    """신규 공지 본문 파싱 및 LLM(gpt-oss-20b) 기반 정책행 생성"""
...
with DAG(
    ...
    description="배달e음 공지사항 정책 변경 수집 및 LLM 기반 분석",
    schedule=SMP_POLICY_BAEDALEUM_TIME,
    start_date=pendulum.datetime(2026, 4, 11, tz="Asia/Seoul"),
    catchup=False,
    max_active_runs=1,
    tags=["01_crawling", "baedaleum", "policy", "daily"],
    default_args={
        "retries": 1,

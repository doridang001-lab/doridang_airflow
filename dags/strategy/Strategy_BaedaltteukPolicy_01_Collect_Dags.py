...
처리 흐름:
1. 가맹점주 공지사항 전 페이지 파싱 (SSR - requests + BeautifulSoup)
2. BoardID 중복 판정 및 신규 목록 생성
3. 신규 공지 본문 수집 및 LLM(gpt-oss-20b) 기반 정책행 생성
4. policy_type 표준 코드 매핑 (8종: 할인/광고/노출/수수료/정산/운영/기능변경/기타)
5. 정책행 누적 저장 및 중복제거(board_id), 정렬(policy_date desc)

실행 시각: 매일 10:20 (KST, 정책 DAG 5분 간격 분산)
데이터 스키마: policy_id, platform, board_id, collected_at, policy_date,
              title, content_summary, policy_type, recommended_action, source_url
"""
...
from modules.transform.utility.schedule import SMP_POLICY_BAEDALTTEUK_TIME
...
pipeline_module_name = "SMP_baedaltteuk_policy_collect"
...
def task_extract_notice_list(**context):
    """배달특급 가맹점주 공지사항 전 페이지에서 공지 목록 파싱"""
...
def task_collect_policy_rows(**context):
    ...
    if not policy_rows:
        print("생성된 정책행이 없습니다. 후속 작업을 스킵합니다.")
...
with DAG(
    ...
    description="배달특급 정책 변경 수집 및 LLM 기반 분석",
    schedule=SMP_POLICY_BAEDALTTEUK_TIME,
    start_date=pendulum.datetime(2026, 4, 10, tz="Asia/Seoul"),
    catchup=False,
    max_active_runs=1,
    tags=["01_crawling", "baedaltteuk", "policy", "daily"],
    default_args={
        "retries": 1,

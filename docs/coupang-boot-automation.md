...
- Chrome 프로필 경로(`Default`) 및 runner 확장 경로(`C:\airflow\coupang_extension_build`)가 유효한지 확인
- 작업 스케줄러의 실행 제한 시간이 짧게 남아 있으면 안 됨. 등록 스크립트는 기본 360분으로 설정한다.
...
- 수집 파일 패턴(`coupangeats_*.csv`)이 `Collect_Data/영업관리부_수집`에 쓰인 뒤
...
- 그 다음 수집 결과 파일(`Collect_Data/영업관리부_수집`) 증가/안정화를 감시한 뒤

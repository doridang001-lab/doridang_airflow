...
let autoRetryTimer  = null;       // 배치 완료 후 자동 재시도 대기 타이머
  ...
  async function processAccount(i, workTabId, skip = {}, targetStoresArg = null) {
    ...
    const targetDate = getRunDateStr(runDateMode);
    const targetDateLabel = getRunDateLabel(runDateMode);
    ...
    if (r.key === 'login') {
      ...
      // inject + sendMessage + 에러 스팬 감지 → 최대 3회 F5 재시도
      const MAX_LOGIN = 3;
      let loginDone = false;
    ...
    if (!skip.orders) {
      ...
      try { await chrome.tabs.sendMessage(workTabId, {type:'COLLECT', source:'batch', targetStores: activeTargetStores, targetDate, targetDateMode: runDateMode}); }
      ...
      if (isHardThrottlePayload(op)) {
        ...
        if (blockedStores.length === 0 && (op.blocked || op.hardThrottle)) blockedStores = [...activeTargetStores];
        log(`[${acc.id}] ${note} — 현재 매장 패스: ${blockedStores.join(' / ') || '미상'}`, 'err');
        recordCategoryResults('orders', acc, targetStores, op, false, note);
        markStoresBlocked(acc, blockedStores, note);
        activeTargetStores = activeTargetStores.filter(store => !blockedStores.some(blocked => storeCompareKey(store) === storeCompareKey(blocked)));
        setRow(i, {orders:'warn', note, current:true, blocked: blockedStores.length === targetStores.length});
        if (activeTargetStores.length === 0) {
          setRow(i, {cmg:'skip', menu:'skip', current:false, promote:true});
    ...
    if (!skip.orders && !ordersOk && !stopRequested && !prevOrdersTimedOut) {
      ...
      if (ordersNav && !stopRequested) {
        ...
        await injectContent(workTabId);
        if (!await waitForOrdersUiReady(workTabId)) {
          log(`[${acc.id}] orders 재시도 UI 준비 대기 타임아웃 — 기존 방식으로 진행`, 'info');
        }
        try { await chrome.tabs.sendMessage(workTabId, {type:'COLLECT', source:'batch', targetStores: activeTargetStores, targetDate, targetDateMode: runDateMode}); }
        ...
        prevOrdersTimedOut = prevOrdersTimedOut || !!rp.timeout;
        partialThrottle = partialThrottle || isPartialThrottlePayload(rp);
        if (isHardThrottlePayload(rp)) {
          const note = '10057: 과도한 요청으로 인해 서비스 이용이 일시적으로 제한되었습니다.';
          let blockedStores = hardThrottleStoreNames(rp);
          if (blockedStores.length === 0 && (rp.blocked || rp.hardThrottle)) blockedStores = [...activeTargetStores];
          log(`[${acc.id}] ${note} — 현재 매장 패스: ${blockedStores.join(' / ') || '미상'}`, 'err');
          recordCategoryResults('orders', acc, targetStores, rp, false, note);
          markStoresBlocked(acc, blockedStores, note);
          activeTargetStores = activeTargetStores.filter(store => !blockedStores.some(blocked => storeCompareKey(store) === storeCompareKey(blocked)));
          setRow(i, {orders:'warn', note, current:true, blocked: blockedStores.length === targetStores.length});
          if (activeTargetStores.length === 0) {
          ...
          ordersOk = categoryPayloadOk(rp, activeTargetStores);
        } else if (rp.skipped) {
          ordersOk = false;
          log(`[${acc.id}] orders 재시도 건너뜀→미완료: ${rp.reason||'브랜드 아님'}`, 'info');
          setRow(i, {orders:'warn'});
        } else {
          ordersOk = categoryPayloadOk(rp, activeTargetStores);
          const retryStatusText = ordersOk ? '완료' : (rp.timeout ? '타임아웃' : rp.stopped ? '중단' : '미완료');
          log(`[${acc.id}] orders 즉시재시도 ${retryStatusText} — ${rp.completedCount||0}/${rp.storeCount||0}개`,
              ordersOk ? 'ok' : 'err');
  ...
  function finishBatch() {
    ...
    // 경고/실패 있으면 5~10분 랜덤 대기 후 자동 재시도 (retryBtn 클릭 = 취소)
    // 단, 사용자가 중단(stopRequested)한 경우엔 자동 재시도 금지 — finishRetry와 동일 가드
    if (incompleteStores().length > 0 && !stopRequested) {

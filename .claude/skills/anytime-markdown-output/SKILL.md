...
- **予約ファイル `index.[lang].md`（各フォルダ必須・OKF 段階開示）**: 索引の構成・自動生成（`scripts/gen-spec-index.mjs`・手書き禁止）・再生成必須の運用は `anytime-doc-authoring` スキル §1.2 に集約した。ドキュメントを追加・更新・改名・削除したら同スキルに従い索引を再生成する。
- `lang` は `ja`（日本語）/ `en`（英語）。
- 分類が不明な場合はユーザーに確認する。
...
| `manual` | `docs/manual/`（コードリポジトリ） | マニュアル・手順書。ユーザー向け操作ガイド・セットアップ手順等。**唯一 `<docsRoot>` の外に置く type**（実利用者がリポジトリのチェックアウトだけで読めるようにするため。根拠は `AGENTS.md`「ドキュメント出力先」節） |
...
**使える値は次の 2 種類だけ**である。C4 要素は `WorkspaceC4ElementProvider` がワークスペース構成から導出しており、手動 C4（`activity_c4_manual_elements`）は実運用で 0 件だからである。
...
コンポーネント ID（`pkg_graph-core/engine` 形式）は**使わない**。ライブ C4 モデルのコンポーネント要素は `community_<n>` 形式であり、この形の ID はどの要素とも一致しない。`/api/docs-index` の repo フィルタは「宣言のいずれかが要素 ID に一致するか、要素 ID の親であるか」で判定するため、**コンポーネント ID だけを宣言した設計書は C4 ビューアから消える**（他に有効な `pkg_` を併記していれば残るが、コンポーネント ID 側は常に空振りする）。
...
`check_alignment` の逆引きは**要素 ID の完全一致**であり、その要素を宣言した設計書は 1 本残らず finding になる。実装の裏付けがない宣言は、そのパッケージを触るたびに恒常的な警報を出し、本当の更新漏れを埋没させる。

**判定の基準は「一次実装者か」である。** 本書が定義する外部から観測できる振る舞いを、そのパッケージが**本体として**実装しているなら宣言する。その実装を呼び出しているだけ・結果を表示しているだけのパッケージは宣言しない。

一次実装者かどうかは、本文が定義する振る舞い（機能名・保持するデータ・コマンド・保存形式）を手掛かりに実装を探して決める。次の 5 つは、その判断を機械的に裏取りするための**補助**である。どれにも当たらないことは、宣言してはいけない理由にならない。
...
> [!WARNING]
> 補助条件だけで機械的に判定しない。component spec は §3.1 の規約により内部シンボルやコードパスを本文へ書かないため、**規約を守った設計書ほど補助条件に当たらない**。機能名でフォルダを切った横断仕様書（`spec/35.mcp/00.bundling/` 等）も「所在」で救えない。実際にこの誤判定で、実装を持つ設計書の宣言を落とした事例がある。
...
#### 3.17.1 整形・検証ルールは §10 に集約

可読性のための整形・記法ルールは本スキル §10（出力後の検証）を単一の真実とする。本節では重複定義しない（値の不一致を防ぐため、個別ルールは §10 のチェックリストへ集約した）。

- **機械的整形**（空行・インデント・行末空白・テーブルのパイプエスケープ）: `format_markdown(path, mode="fix")`（mcp-markdown）が自動修正する。手書きで揃えない。対象と挙動は §10.1。
- **意味判断・構造**（固有名称のコードスパン化／補足の引用ブロック化／複雑ロジックの Mermaid 図化／ハードブレーク／箇条書きの階層）: §10.2〜10.3 のチェックリストに従う。閾値は §10 側にのみ書く。

#### 3.17.2 ファイル粒度と部分読み最適化（AI エージェント可読性）

<!-- 旧 3.17.4。3.17.2〜3.17.3 の §10 集約（§3.17.1 参照）に伴い再採番 -->
...
- **既存コンセプトはインラインリンクで必ず結ぶ（必須）**: 本文で他の設計書・テーブル・パッケージ等、既に文書化された概念に言及する際は、ルート相対リンク（`[customers](/tables/customers.md)` 等）で必ず接続する。frontmatter の `related` だけに頼らず本文リンクでも結ぶことで、ドキュメント群がリンクグラフ（note-graph）を形成し、AI エージェントが関連文書を最小コストで辿れる（OKF のグラフ構造原則）。
...
type 別の記載内容（component spec の外部仕様原則・spec/e2e 2 ファイル分離・E2E シナリオ表・見出し粒度・索引運用を含む）は `anytime-doc-authoring` スキルへ移管した（値の不一致を防ぐため本仕様では重複定義しない。§3.17.1 の §10 への集約と同じ方針）。本スキルは構文・フロントマター・整形（どう書くか）と、出力後の検証（§10）を扱う。

## 10. 出力後の検証

<!-- 2026-08-22: 旧 `anytime-markdown-check` スキルを統合。書く規約（§1〜§9）と出力後の検証を同一スキルへ集約した。 -->

ファイル出力後、(1) 機械的整形を `format_markdown` で適用し、(2) 自動化できない意味判断を以下のチェックリストで確認する。

### 10.1 機械的整形（format_markdown に委譲）
...
- [ ] 変数名、ファイル名、ボタン名、テーブル名等のシステム固有名称がコードスパン（`` ` ``）で囲まれているか（§3.5）
- [ ] 補足情報が引用ブロック（`>`）で本文と分離されているか（§3.8）
- [ ] 複雑なロジックが Mermaid 図で図解されているか（該当する場合。§5）
- [ ] 「。」の後に文章が続く場合、意図通りのハードブレーク／段落分けになっているか（`format_markdown` は `hardBreakAfterPeriod` で警告するのみ・自動修正しない。§3.14）
- [ ] コードスパン内のバックスラッシュが不要にエスケープされていないか（リテラル扱い。§3.12）

### 10.3 構造（手動・再設計が必要）

機械的には修正できない。`format_markdown` は警告のみ。

- [ ] 箇条書きの深さが2階層以内か（超える場合は子見出しで再設計。`format_markdown` は `nestDepth` で警告するのみ。§3.6）
- [ ] Markdown 記法文字がテーブルセルを壊していないか（§3.4）

...
`<docsRoot>/` 配下の日本語ドキュメント（spec / tech / test / manual / proposal / plan / review / report のすべて）を**書く・推敲する**ときの文体・論証・推敲規範。type を問わず適用する。`anytime-doc-authoring` 本文の「何を書くか」に対して、本ファイルは「どういう文章で書くか」を定める。

> 構文・整形（見出し前後の空行・テーブルパイプ・箇条書きインデント等の機械的整形）は `anytime-markdown-output` / `format_markdown` が担う。本ファイルは機械では判定しにくい**文章の質**（論証・冗長・語り・LLM 口調）を扱う。
>
> 出典: k16shikano「日本語技術文書の文章規範」<https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d>（参照日: 2026-07-18）。gist のライセンス条項を確認のうえ、規範を参考に本プロジェクト向けに再構成した派生物である（逐語転載ではない）。ライセンスが不明な場合は著者への確認可否を検討する。
>
> 読み物寄りの type（tech の解説記事、manual の解説部、proposal / report の論述部）では、本ファイルに加えて `cognitive-rhythm.ja.md`（認知リズム、すなわち緩急の設計。文の拍・段落の密度波形・冒頭と節の入り方・未回収の緊張の管理・緩みと駄文の判別）を併用する。衝突した場合は本ファイルの「演出の抑制」「冗長の排除」「LLM っぽい表現の禁止」を優先する。
...
- 例の節の中でも、**その節の問い・帰結に関係しない**過剰な詳細だけを省く。議論に必要な具体は残す。省略の典型は、装飾的精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名。
...
- 術語の響きを持つ語を、術語でない場面に流用しない。「届くまでの流れ」「あいだに何があるか」のように普通の言い方で書く。
...
- 慣用表現をひねった言い回しや、指す内容が一意に決まらない比喩を使わない。平易な動詞でそのまま言う（「身につく」「気付く機会が減る」）。

## LLM っぽい表現の禁止

LLM が大量生成する、中身のない型に誘惑されない。書き上げたら、この節で点検する。術語を議論に使うのはよいが、空虚な装飾として使うのが問題である。
...
- 節で扱っている対象を指す名詞句でもよい。
- 見出しが疑問形か断定形かは問わない。問うのは、扱う対象や読者の持つ問いを指しているかである。本文のトーンに合うほうを選ぶ。
...
出力後、上の各節に照らして点検する。特に:

- [ ] 一段落一トピックか。段落先頭で前段落との論理関係を接続表現で示したか
- [ ] 推量・疑念・反実仮想を機械的に断定へ変えていないか。逆に、根拠のない緩和を残していないか
- [ ] 異なるものを「同じ」とまとめ、複数要因を単一原因へ還元していないか。因果には機構を一文添えたか
- [ ] 章・節をまたいで概念・分類・用語の地位が揃っているか
- [ ] LLM 口調（予告と総括・正面から系・空虚な形容/動詞・接続の型・中身のない強調）を使っていないか
- [ ] 同じ主張の言い換え反復・隣接節の役割重複がないか
- [ ] 見出しが内容を特定しているか（手順名・情報量ゼロ・オチのネタバレになっていないか）
- [ ] 後で参照しない固有名・過剰な装飾的精度を持ち込んでいないか

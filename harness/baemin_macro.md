...
## 금지사항
- 과거 요청은 `DB_Beamin_Macro_Backfill_*` 전용 DAG와 `history` 큐로 보낸다. 신규 등록은 `modules.transform.utility.workload.route_trigger`를 사용해 상한·중복·보류 보존 규칙을 적용한다.
- 백필 수집·재시도·업로드·검증의 자동 등록은 전체 queued/running 합계 1건이다. 생산자는 보류 요청만 저장하며, 자원 제어기의 배정기가 부모 종료 후 후속 작업을 우선 등록한다. 일반 수집과 야간 통합은 이 상한에 포함하지 않는다.
- UI/API의 `dagrun_failed` 감사 기록이 있는 실행과 그 후속 요청은 자동으로 되살리지 않는다. 백필 직접 `trigger_dag` 및 구형 watcher의 직접 clear를 사용하지 않는다. UI에서 직접 실행/clear하면 자동 등록 제한을 우회하므로 운영 자동화는 반드시 공통 배정 경로를 사용한다.
- 기존 대기열 전환은 백필 DAG를 일시 정지한 뒤 `history_admission.defer_unstarted`로 미시작 실행만 처리한다. 워커에 이미 배정된 태스크는 자동 전환하지 않으며, 실행 기록과 conf를 보존한다. 원래 일시 정지 상태를 복원한 후 배정 제한을 해제한다.
- 원본 DAG를 복제할 때 `weight_rule`에 문자열을 직접 재대입하지 않는다. 생성자 밖에서는 등록된 우선순위 전략 객체를 사용하고, DAG 직렬화까지 검증한다.

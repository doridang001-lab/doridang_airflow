...
쿠팡이츠 파트너 운영센터(zendesk) 공지사항에서 정책 변경 사항을 수집하고
LLM 기반 분석을 수행합니다.
...
from modules.transform.utility.paths import COUPANG_POLICY_CSV_PATH

logger = logging.getLogger(__name__)

# 쿠팡이츠 zendesk API (HTML 직접 접근은 403, API 사용)
COUPANG_BASE_URL = "https://coupangeatsoperations.zendesk.com"
COUPANG_SECTION_ID = "19127370529817"
COUPANG_NOTICE_API = (
    f"{COUPANG_BASE_URL}/api/v2/help_center/ko/sections/{COUPANG_SECTION_ID}/articles.json"
    "?sort_by=created_at&sort_order=desc&per_page=30"
)
COUPANG_ARTICLE_API = f"{COUPANG_BASE_URL}/api/v2/help_center/ko/articles/{{article_id}}.json"

# 공통 정책 타입 표준 코드 (8종, 플랫폼 공용)
POLICY_TYPES = ["할인", "광고", "노출", "수수료", "정산", "운영", "기능변경", "기타"]
DEFAULT_POLICY_TYPE = "기타"

# LLM 엔드포인트 (환경변수, 배민과 동일)
...
def _normalize_policy_date(raw_date: str) -> str:
    """공지 날짜 텍스트 또는 ISO8601 속성값을 YYYY-MM-DD로 정규화한다."""
    text = (raw_date or "").strip()
    # ISO8601: 2024-03-15T00:00:00Z 또는 2024-03-15
    iso_match = re.match(r"(\d{4})-(\d{2})-(\d{2})", text)
    if iso_match:
        return f"{iso_match.group(1)}-{iso_match.group(2)}-{iso_match.group(3)}"
    # 한국식: 2024. 03. 15. / 2024.03.15
...
def _infer_policy_type(text: str) -> str:
    """제목/요약 텍스트에서 공통 정책유형을 우선순위 기반으로 판정한다.
    ...
    # 할인 (쿠팡이츠: 와우 추가)
    if any(k in t for k in ["할인", "즉시할인", "쿠폰", "배달비 지원", "이벤트", "와우"]):
        return "할인"

    # 수수료 (쿠팡이츠: 중개이용료, 요금제 추가)
    if any(k in t for k in ["수수료", "중개이용료", "우대수수료", "요금제", "요금", "비용"]):
        return "수수료"

    # 광고
    if any(k in t for k in ["광고", "마케팅", "프로모션"]):
        return "광고"
...
def _refine_summary(title: str, summary: str, content_text: str) -> str:
    ...
    low_quality_markers = ["안녕하세요", "공지입니다", "안내드립니다", "쿠팡이츠입니다"]
...
def _build_action(policy_type: str, title: str) -> str:
    ...
    # 할인 관련
    if "와우" in t:
        return "와우 회원 대상 혜택을 적극 활용해 객단가 높은 상품을 홍보하세요"
    if "즉시할인" in t:
        return "즉시할인 메뉴를 설정하고 배너 노출로 객단가 증대를 유도하세요"
    if "배달비" in t and ("지원" in t or "면제" in t):
        return "배달비 지원 기간에 배달 주문을 유도하는 프로모션을 강화하세요"
    if "이벤트" in t:
        return "이벤트 기간에 신메뉴 및 고수익 상품을 집중 판매하세요"

    # 수수료 관련
    if "중개이용료" in t or "요금제" in t:
        return "중개이용료/요금제 변경을 반영해 상품 가격과 마진 구조를 즉시 조정하세요"
    if "수수료" in t or "요금" in t or "비용" in t:
        return "수수료 변경을 반영해 상품 가격과 마진 구조를 조정하세요"

    # 기능변경 관련
    if "픽업" in t:
        return "픽업 관련 설정을 확인하고 픽업 상품 가격 전략을 수립하세요"
    if "포장" in t:
        return "포장 관련 설정을 업데이트하고 포장 주문 운영 방침을 점검하세요"
...
def extract_notice_list(**context) -> List[Dict[str, Any]]:
    """
    쿠팡이츠 zendesk Help Center API로 공지 목록 수집
    (HTML 직접 접근은 403 → API 사용)

    Returns:
        List[Dict]: [{'notice_id': str, 'title': str, 'policy_date': str, 'url': str, 'platform': 'coupang'}]
    """
    logger.info("=" * 60)
    logger.info("[1단계] 쿠팡이츠 공지 목록 수집 시작 (Zendesk API)")
    logger.info(f"API: {COUPANG_NOTICE_API}")

    try:
        resp = requests.get(COUPANG_NOTICE_API, headers=REQUEST_HEADERS, timeout=30)
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
        logger.warning(f"공지 목록 API 수집 실패: {e}")
        return []

    articles = data.get("articles", [])
    if not articles:
        logger.warning("API 응답에 articles 항목이 없습니다.")
        return []

    logger.info(f"API 반환 공지 수: {len(articles)}")

    notices = []
    for article in articles:
        try:
            notice_id = str(article["id"])
            title = (article.get("title") or "").strip()
            if not title:
                continue

            # created_at: ISO8601 (예: "2026-04-01T03:00:00Z")
            created_at = article.get("created_at") or article.get("updated_at", "")
            policy_date = _normalize_policy_date(created_at) if created_at else pendulum.now("Asia/Seoul").format("YYYY-MM-DD")

            url = article.get("html_url") or f"{COUPANG_BASE_URL}/hc/ko/articles/{notice_id}"
...
def detect_new_notices(notice_list: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    ...
    # 기존 CSV 로드
    if COUPANG_POLICY_CSV_PATH.exists():
        try:
            existing_df = pd.read_csv(COUPANG_POLICY_CSV_PATH, encoding="utf-8-sig")
            logger.info(f"기존 정책 데이터: {len(existing_df)}행")
            existing_keys = set(
                existing_df.apply(
                    lambda row: f"{row['title']}|{row['policy_date']}", axis=1
                ).tolist()
            )
        except Exception as e:
    ...
    # 신규 공지 필터링 (최신순 목록 가정 - 첫 중복 발견 시 스캔 중단)
    new_notices = []
    for notice in notice_list:
        key = f"{notice['title']}|{notice['policy_date']}"
        if key in existing_keys:
            logger.info(
                f"중복 공지 발견으로 스캔 중단: {notice['title']} ({notice['policy_date']})"
            )
            break
...
def collect_policy_rows(new_notices: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    ...
    for i, notice in enumerate(new_notices, 1):
        ...
        # 본문 수집 (Zendesk API)
        article_api_url = COUPANG_ARTICLE_API.format(article_id=notice["notice_id"])
        try:
            resp = requests.get(article_api_url, headers=REQUEST_HEADERS, timeout=30)
            resp.raise_for_status()
            article_data = resp.json().get("article", {})
        except Exception as e:
            logger.warning(f"본문 API 수집 실패 - 스킵합니다: {e}")
            continue

        body_html = article_data.get("body", "")
        if not body_html:
            logger.warning("본문 body가 비어 있습니다. 스킵합니다.")
            continue
        ...
        prompt = f"""다음은 쿠팡이츠 파트너 운영센터 공지사항입니다. 아래 3가지를 JSON 형식으로 추출하세요:
...
3. recommended_action: 사업자가 바로 실행할 수 있는 투자/운영 제안 문장으로 작성 (15~45자)
    예시: "중개이용료 변경을 반영해 상품 가격과 마진 구조를 조정하세요"
...
"""
        ...
        # LLM 분석 (OpenAI 호환 -> Ollama 네이티브 순으로 시도, 2회 실패 시 규칙 기반 대체)
        llm_result = None
        if llm_enabled:
            for attempt in range(2):
                try:
                    ...
                    llm_result["policy_type"] = (
                        inferred_type if inferred_type in POLICY_TYPES else DEFAULT_POLICY_TYPE
                    )
                    llm_result["content_summary"] = _refine_summary(
                        notice["title"], raw_summary, content_text
                    )
                    llm_result["recommended_action"] = _build_action(
                        llm_result["policy_type"], notice["title"]
                    )
        ...
        policy_row = {
            "policy_id": f"coupang_{notice['notice_id']}",
            "platform": "coupang",
...
def normalize_policy_types(policy_rows: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    """
    공통 policy_type 표준 코드 매핑 (쿠팡이츠 특이 키워드 포함)
    ...
    # 쿠팡이츠 전용 키워드 포함 매핑 테이블
    type_mapping = {
        "할인": ["할인", "프로모션", "이벤트", "혜택", "와우"],
        "광고": ["광고", "마케팅"],
        "노출": ["노출", "순위", "검색", "배너"],
        "정산": ["정산", "수익", "지급", "무이자", "할부"],
        "수수료": ["수수료", "요금", "비용", "중개이용료", "요금제"],
        "운영": ["운영", "관리", "정책", "규정"],
        "기능변경": ["기능", "변경", "업데이트", "개선", "픽업", "포장"],
        "기타": ["기타", "안내", "공지"],
    }
...
def save_policy_csv(normalized_rows: List[Dict[str, Any]], **context) -> str:
    ...
    if not normalized_rows:
        ...
        return str(COUPANG_POLICY_CSV_PATH)
    ...
    # CSV 저장
    COUPANG_POLICY_CSV_PATH.parent.mkdir(parents=True, exist_ok=True)
    merged_df.to_csv(COUPANG_POLICY_CSV_PATH, index=False, encoding="utf-8-sig")

    logger.info(f"저장 완료: {COUPANG_POLICY_CSV_PATH}")
...
def write_policy_log(**context) -> str:
    """policy/log.parquet에 실행 이력 기록 (coupang)"""
    from modules.transform.utility.paths import POLICY_LOG_PATH

    ti = context["ti"]
    file_path = ti.xcom_pull(task_ids="task_save_policy_csv") or str(COUPANG_POLICY_CSV_PATH)
    ...
    try:
        total = len(pd.read_csv(COUPANG_POLICY_CSV_PATH, encoding="utf-8-sig")) if COUPANG_POLICY_CSV_PATH.exists() else 0
    except Exception:
        total = 0
    ...
    try:
        ...
        logger.info(f"log.parquet 기록 완료: {POLICY_LOG_PATH} | coupang inserted={inserted} result={result}")
        return f"log.parquet 기록 완료: coupang inserted={inserted} result={result}"
    except Exception as e:

...
배민 CEO 공지사항에서 정책 변경 사항을 수집하고 LLM 기반 분석을 수행합니다.
...
from modules.transform.utility.paths import BAEMIN_POLICY_CSV_PATH

logger = logging.getLogger(__name__)

# 배민 CEO 공지 URL
BAEMIN_NOTICE_URL = "https://ceo.baemin.com/notice?category=서비스%20안내&page=1"
...
def _infer_policy_type(text: str) -> str:
    """제목/요약 텍스트에서 정책유형을 우선순위 기반으로 재판정한다.
    ...
    # 정산: 정산/지급/입금/무이자/할부 등 매우 명확
    if any(k in t for k in ["정산", "지급", "입금", "무이자", "할부", "이자", "결제일"]):
        return "정산"
    
    # 할인: 할인/쿠폰/배달비지원 등 할인성 지원
    if any(k in t for k in ["할인", "즉시할인", "쿠폰", "배달비 지원", "배민위크", "이벤트"]):
        return "할인"
    
    # 수수료: 수수료/요금 변경
    if any(k in t for k in ["수수료", "중개이용료", "우대수수료", "요금", "비용"]):
        return "수수료"
    
    # 광고: 광고투자/마케팅
    if any(k in t for k in ["우리가게클릭", "우가클", "광고", "마케팅", "배민클릭"]):
        return "광고"
    
    # 기능변경: 신규기능/설정/업데이트
    if any(k in t for k in ["기능", "업데이트", "개선", "도입", "설정", "추가", "변경"]):
        return "기능변경"
    
    # 운영: 운영정책/휴무/서비스명칭
    if any(k in t for k in ["운영", "휴무", "영업", "정책", "서비스 명칭", "연휴"]):
        return "운영"
...
def _refine_summary(title: str, summary: str, content_text: str) -> str:
    ...
    low_quality_markers = ["안녕하세요", "공지입니다", "안내드립니다", "배달의민족입니다"]
...
def _build_action(policy_type: str, title: str) -> str:
    """정책유형/제목 기반 실행 제안 문장을 생성한다. 제목의 세부 내용을 반영해 구체적으로."""
    t = _clean_text(title)
    ...
    if "배민위크" in t or "이벤트" in t:
        return "할인 이벤트 기간에 신메뉴 및 고수익 상품을 집중 판매하세요"
    
    # 광고 관련
    if "우리가게클릭" in t or "우가클" in t:
        return "우가클 지원 구간에 예산을 집중 투자하고 클릭 당 매출을 메트릭으로 관리하세요"
    if "광고" in t and ("명칭" in t or "변경" in t):
        return "광고 서비스 변경 내용을 파악하고 광고 설정을 즉시 업데이트하세요"
    
    # 수수료 관련
    if "수수료" in t or "요금" in t or "비용" in t:
        return "수수료 변경을 반영해 상품 가격과 마진 구조를 조정하세요"
    
    # 기능변경 관련
    if "픽업" in t or "픽업 가격" in t:
        return "픽업 가격 설정 기능을 활성화하고 픽업 상품 가격 전략을 수립하세요"
    ...
    if "서비스 명칭" in t or "서비스명칭" in t:
        return "서비스 명칭 변경 사항을 매장 앱과 공지에 반영하세요"
...
def _normalize_policy_date(raw_date: str) -> str:
    ...
    # 예: 2026. 03. 26. / 2026.03.26 / 2026-03-26
...
def _fetch_html(url: str, wait_selector: str | None = None) -> str:
    """HTML을 가져온다. requests 우선, CSR 페이지면 Playwright로 렌더링한다."""
    try:
        ...
        # 공지 목록/본문 핵심 마커가 없으면 CSR 셸로 판단
        has_list_marker = "noticeItemsWrap" in text or "href=\"/notice/" in text
        has_content_marker = "commonStyled__HtmlContent" in text or "class=\"content" in text
        if has_list_marker or has_content_marker:
            return text
    except Exception as e:
        logger.warning(f"requests HTML 수집 실패: {e}")
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page = browser.new_page(user_agent=REQUEST_HEADERS["User-Agent"], locale="ko-KR")
        ...
        logger.error(f"Playwright 렌더링 실패: {e}")
        return ""
...
def extract_notice_list(**context) -> List[Dict[str, Any]]:
    ...
    Returns:
        List[Dict]: [{"notice_id": str, "title": str, "policy_date": str, "url": str}, ...]
    """
    logger.info("=" * 60)
    logger.info("[1단계] 배민 공지 목록 파싱 시작")
    logger.info(f"URL: {BAEMIN_NOTICE_URL}")

    html = _fetch_html(BAEMIN_NOTICE_URL, wait_selector='ul[data-testid="noticeItemsWrap"]')
    if not html:
        logger.warning("공지 목록 HTML을 가져오지 못했습니다.")
        return []

    soup = BeautifulSoup(html, 'html.parser')
    notices = []
    
    # 실제 배민 공지 DOM 기준 + fallback
    notice_items = (
        soup.select('ul[data-testid="noticeItemsWrap"] > li')
        or soup.select('ul[data-testid="noticeItemsWrap"] li')
        or soup.select('a[href*="/notice/"]')
        or soup.select('.notice-list .notice-item')
    ...
    if not notice_items:
        logger.warning("공지 목록 선택자를 찾지 못했습니다. 페이지 구조를 확인하세요.")
        return []
    
    logger.info(f"파싱된 공지 항목 수: {len(notice_items)}")
    
    for item in notice_items:
        try:
            # notice_items가 a태그 리스트일 수도 있으므로 자기 자신 포함 처리
            if getattr(item, "name", "") == "a":
                link_elem = item
            else:
                link_elem = item.select_one('a[href*="/notice/"]') or item.select_one('a')
            if not link_elem:
                continue

            # 제목 추출: 실제 클래스(text-ellipsis) 우선
            title_elem = (
                link_elem.select_one('.text-ellipsis')
                or link_elem.select_one('div[class*="styled__Title"] span:last-child')
                or item.select_one('.title')
                or item.select_one('.notice-title')
                or item.select_one('td:nth-child(2)')
            )
            ...
            if not isinstance(href, str) or not href:
                continue
            url = href if href.startswith('http') else f"https://ceo.baemin.com{href}"
            
            # 공지 ID 추출 (URL에서 추출)
            notice_id = url.split('/')[-1].split('?')[0]
            
            # 날짜 추출
            date_elem = (
                link_elem.select_one('div[class*="styled__DateTextWrapper"] span')
                or item.select_one('.date')
                or item.select_one('.notice-date')
                or item.select_one('td:nth-child(3)')
                or item.select_one('.created-at')
            )
            policy_date = _normalize_policy_date(date_elem.get_text(strip=True) if date_elem else "")
...
def detect_new_notices(notice_list: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    ...
    # 기존 CSV 로드
    if BAEMIN_POLICY_CSV_PATH.exists():
        try:
            existing_df = pd.read_csv(BAEMIN_POLICY_CSV_PATH, encoding='utf-8-sig')
            logger.info(f"기존 정책 데이터: {len(existing_df)}행")
            
            # title+policy_date 조합으로 중복 판정
            existing_keys = set(
    ...
    # 신규 공지 필터링 (목록이 최신순이라고 가정하고 최초 중복 발견 시 중단)
    new_notices = []
...
def collect_policy_rows(new_notices: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    ...
    for i, notice in enumerate(new_notices, 1):
        ...
        # 본문 수집
        try:
            detail_html = _fetch_html(notice['url'], wait_selector='div.commonStyled__HtmlContent-sc-1rv6nps-0, div.content.mb-5')
            if not detail_html:
                logger.warning("본문 HTML을 가져오지 못했습니다. 스킵합니다.")
                continue

            soup = BeautifulSoup(detail_html, 'html.parser')
            
            # 본문 선택자 (fallback 포함)
            content_elem = soup.select_one('.notice-content') or \
                          soup.select_one('.content') or \
                          soup.select_one('.board-content') or \
                          soup.select_one('article')
            
            if not content_elem:
                logger.warning(f"본문을 찾지 못했습니다. 스킵합니다.")
                continue
        ...
        prompt = f"""다음은 배달의민족 공지사항입니다. 아래 3가지를 JSON 형식으로 추출하세요:
...
3. recommended_action: 사업자가 바로 실행할 수 있는 투자/운영 제안 문장으로 작성 (15~45자)
    예시: "배민에서 우가클 100 지원하니 우가클에 더 투자하세요"
...
"""
        ...
        if llm_enabled:
            for attempt in range(2):
                try:
                    ...
                    # 우선 _infer_policy_type으로 기본 타입 결정
                    inferred_type = _infer_policy_type(title_for_type)

                    # 요약도 시도해보기 (제목만으로 부족하면)
                    ...
                    # 액션 문장은 항상 유형/제목 기반으로 생성해 품질을 고정한다.
        ...
        # 정책행 생성
        policy_row = {
            'policy_id': f"baemin_{notice['notice_id']}",
            'platform': 'baemin',
...
def save_policy_csv(normalized_rows: List[Dict[str, Any]], **context) -> str:
    ...
    if not normalized_rows:
        ...
        return str(BAEMIN_POLICY_CSV_PATH)
    ...
    # CSV 저장
    BAEMIN_POLICY_CSV_PATH.parent.mkdir(parents=True, exist_ok=True)
    merged_df.to_csv(BAEMIN_POLICY_CSV_PATH, index=False, encoding='utf-8-sig')
    
    logger.info(f"✅ 저장 완료: {BAEMIN_POLICY_CSV_PATH}")
...
def write_policy_log(**context) -> str:
    """policy/log.parquet에 실행 이력 기록 (baemin)"""
    from modules.transform.utility.paths import POLICY_LOG_PATH

    ti = context["ti"]
    file_path = ti.xcom_pull(task_ids="task_save_policy_csv") or str(BAEMIN_POLICY_CSV_PATH)
    ...
    try:
        total = len(pd.read_csv(BAEMIN_POLICY_CSV_PATH, encoding="utf-8-sig")) if BAEMIN_POLICY_CSV_PATH.exists() else 0
    except Exception:
        total = 0
    ...
    try:
        ...
        logger.info(f"log.parquet 기록 완료: {POLICY_LOG_PATH} | baemin inserted={inserted} result={result}")
        return f"log.parquet 기록 완료: baemin inserted={inserted} result={result}"
    except Exception as e:

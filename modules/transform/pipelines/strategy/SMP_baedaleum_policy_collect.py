...
배달e음(인천사랑상품권) 공지사항에서 정책 변경 사항을 수집하고 LLM 기반 분석을 수행합니다.

처리 흐름:
1. REST API로 공지 목록 수집 (incheoneum.or.kr - 번호 있는 항목만)
2. title+policy_date 중복 판정 및 신규 목록 생성
3. 공지 본문(list 응답 포함) 파싱 및 LLM(gpt-oss-20b) 기반 정책행 생성
...
from modules.transform.utility.paths import BAEDALEUM_POLICY_CSV_PATH

logger = logging.getLogger(__name__)

# 배달e음 REST API (CSR 페이지 - REST API 직접 호출)
BAEDALEUM_API_URL = (
    "https://incheoneum.or.kr/api/board/notices"
    "?page=1&perPage=10&searchKeyword=&category=title"
)
BAEDALEUM_VIEW_URL = "https://incheoneum.or.kr/info/notice"
...
REQUEST_HEADERS = {
    ...
    "Referer": "https://incheoneum.or.kr/",
}
...
def _infer_policy_type(text: str) -> str:
    ...
    if any(k in t for k in ["정산", "지급", "입금", "무이자", "할부", "이자", "결제일"]):
        return "정산"
    if any(k in t for k in ["캐시백", "할인", "즉시할인", "쿠폰", "배달비 지원", "이벤트", "프로모션", "혜택", "복드림"]):
        return "할인"
    ...
    if any(k in t for k in ["기능", "업데이트", "개선", "도입", "설정", "추가", "변경"]):
        return "기능변경"
    if any(k in t for k in ["운영", "휴무", "영업", "정책", "연휴", "장애", "오류"]):
        return "운영"
...
def _refine_summary(title: str, summary: str, content_text: str) -> str:
    ...
    low_quality_markers = ["안녕하세요", "공지입니다", "안내드립니다", "배달e음입니다"]
...
def _build_action(policy_type: str, title: str) -> str:
    ...
    if "캐시백" in t and ("상향" in t or "인상" in t):
        return "캐시백 요율 상향 기간에 인천사랑상품권 결제를 적극 유도하세요"
    if "캐시백" in t and ("중지" in t or "중단" in t or "혜택 중지" in t):
        return "캐시백 혜택 조건을 확인하고 매출 기준을 관리하세요"
    if "캐시백" in t:
        return "캐시백 혜택 기간에 인천사랑상품권 결제를 집중 홍보하세요"
    if "이벤트" in t or "복드림" in t or "프로모션" in t:
        return "이벤트 기간에 상품권 결제 유도 안내를 매장에 부착하세요"
    if "장애" in t or "오류" in t:
        return "결제 장애 복구 여부를 확인하고 고객 문의에 대응하세요"
    if "상생가맹점" in t:
        return "상생가맹점 혜택 조건을 확인하고 가입 신청 여부를 검토하세요"

    action_map = {
        "할인": "할인/캐시백 혜택 기간에 인천사랑상품권 결제를 적극 유도하세요",
...
def extract_notice_list(**context) -> List[Dict[str, Any]]:
    """
    배달e음 REST API로 공지 목록 수집 (번호 있는 항목만)

    Returns:
        List[Dict]: [{"notice_id": str, "title": str, "policy_date": str,
                      "url": str, "platform": "baedaleum", "content_html": str}, ...]
    """
    logger.info("=" * 60)
    logger.info("[1단계] 배달e음 공지 목록 수집 시작 (REST API)")
    logger.info(f"URL: {BAEDALEUM_API_URL}")

    try:
        resp = requests.get(BAEDALEUM_API_URL, headers=REQUEST_HEADERS, timeout=30)
        if not resp.ok:
    ...
    # data.contents: 번호 있는 일반 공지 (fixedContents 핀 고정 제외)
    ...
    logger.info(f"API 응답: 총 {total_count}건 중 {len(contents)}건 수신 (번호 있는 공지)")
...
def detect_new_notices(notice_list: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    ...
    if BAEDALEUM_POLICY_CSV_PATH.exists():
        try:
            existing_df = pd.read_csv(BAEDALEUM_POLICY_CSV_PATH, encoding='utf-8-sig')
...
def collect_policy_rows(new_notices: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    """
    신규 공지 본문 파싱 및 LLM 기반 정책행 생성
    (배달e음은 list 응답에 contents HTML 포함 → 별도 상세 API 불필요)
    ...
    """
    logger.info("=" * 60)
    logger.info("[3단계] 정책 본문 파싱 및 LLM 분석 시작")
    ...
    for i, notice in enumerate(new_notices, 1):
        ...
        # 본문 파싱 (list 응답에 포함된 contents HTML)
        ...
        if len(content_text) < 10:
            logger.warning(f"본문이 없거나 너무 짧습니다({len(content_text)}자). 제목으로 대체합니다.")
            content_text = notice['title']

        logger.info(f"  본문 파싱 완료: {len(content_text)}자")

        prompt = f"""다음은 배달e음(인천사랑상품권) 공지사항입니다. 아래 3가지를 JSON 형식으로 추출하세요:
...
3. recommended_action: 사업자가 바로 실행할 수 있는 투자/운영 제안 문장으로 작성 (15~45자)
    예시: "캐시백 혜택 기간에 인천사랑상품권 결제를 집중 홍보하세요"
...
"""
        ...
        policy_row = {
            'policy_id': f"baedaleum_{notice['notice_id']}",
            'platform': 'baedaleum',
...
def normalize_policy_types(policy_rows: List[Dict[str, Any]], **context) -> List[Dict[str, Any]]:
    ...
    type_mapping = {
        '할인': ['할인', '프로모션', '이벤트', '혜택', '캐시백'],
        '광고': ['광고', '마케팅'],
        '노출': ['노출', '순위', '검색', '배너'],
        '정산': ['정산', '수익', '지급', '무이자', '할부'],
        '수수료': ['수수료', '요금', '비용', '중개이용료', '요금제'],
        '운영': ['운영', '관리', '정책', '규정', '장애', '오류'],
        '기능변경': ['기능', '변경', '업데이트', '개선'],
        '기타': ['기타', '안내', '공지'],
    }
...
def save_policy_csv(normalized_rows: List[Dict[str, Any]], **context) -> str:
    ...
    if not normalized_rows:
        ...
        return str(BAEDALEUM_POLICY_CSV_PATH)
    ...
    BAEDALEUM_POLICY_CSV_PATH.parent.mkdir(parents=True, exist_ok=True)
    merged_df.to_csv(BAEDALEUM_POLICY_CSV_PATH, index=False, encoding='utf-8-sig')

    logger.info(f"✅ 저장 완료: {BAEDALEUM_POLICY_CSV_PATH}")
...
def write_policy_log(**context) -> str:
    """policy/log.parquet에 실행 이력 기록 (baedaleum)"""
    from modules.transform.utility.paths import POLICY_LOG_PATH

    ti = context["ti"]
    file_path = ti.xcom_pull(task_ids="task_save_policy_csv") or str(BAEDALEUM_POLICY_CSV_PATH)
    ...
    try:
        total = len(pd.read_csv(BAEDALEUM_POLICY_CSV_PATH, encoding="utf-8-sig")) if BAEDALEUM_POLICY_CSV_PATH.exists() else 0
    except Exception:
        total = 0
    ...
    try:
        ...
        logger.info(f"log.parquet 기록 완료: {POLICY_LOG_PATH} | baedaleum inserted={inserted} result={result}")
        return f"log.parquet 기록 완료: baedaleum inserted={inserted} result={result}"
    except Exception as e:

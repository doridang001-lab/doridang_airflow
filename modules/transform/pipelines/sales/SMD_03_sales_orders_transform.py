...
def preprocess_join_orders_with_stores(
    ...
    **context
    ...
    if 'store_name' in orders_df.columns:
        ...
        orders_df['store_names'] = normalize_store_names(orders_df['store_names'])
        print(f"[주문데이터] store_names (끝 2단어, 매핑 적용) 샘플: {orders_df['store_names'].head(5).tolist()}")
    else:
...
<p style="color: orange;">⚠️ 상세 데이터 없음</p>
    ...
    try:
        ...
        if '매장명' in orders_df.columns:
            orders_df['매장명'] = normalize_store_names(orders_df['매장명'])
...
def send_alert_email(**context):
    ...
    for (manager, email), group in managers:
        ...
        score_cols = ['매장명', 'status', 'score', 'total_amount', 
                  ...
                  'ma_14', 'ma_28', 'amt_curr_mtd', 'amt_prev_mtd',
        ...
        interest_count = len(store_details[store_details['status'] == '관심'])
...
"""
        ...
        for idx, (_, row) in enumerate(store_details.iterrows()):
            ...
            amt_curr_mtd = row.get('amt_curr_mtd', None)
            amt_prev_mtd = row.get('amt_prev_mtd', None)
                                    ...
                                    <td style="padding: 15px;">
                                        ...
                                        <table width="100%" cellpadding="8" cellspacing="0" border="0" style="font-size: 12px;">
                                            ...
                                            <tr>
                                                ...
                                                <td style="text-align: center; padding: 8px; font-size: 11px;">{int(amt_curr_mtd) if pd.notna(amt_curr_mtd) else '-':,} vs {int(amt_prev_mtd) if pd.notna(amt_prev_mtd) else '-':,}</td>
                    ...
                    <tr>
                        <td style="padding: 25px 30px; background: #f8f9fa; border-top: 1px solid #eee;">
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size: 12px; margin-top: 8px;">
                                <tr>
                                    <td style="padding: 5px 0;">
                                        • <strong style="color: #e74c3c;">위험 (5~6점)</strong>: 즉시 알림
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding: 5px 0;">
                                        • <strong style="color: #f39c12;">주의 (3~4점)</strong>: 알림 없음
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding: 5px 0;">
                                        • <strong style="color: #3498db;">관심 (2점)</strong>: 모니터링
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding: 5px 0;">
                                        • <strong style="color: #27ae60;">정상 (0~1점)</strong>: 알림 없음
                                    </td>
                                </tr>
                            </table>
                            
                            <p style="margin: 15px 0 0 0; font-size: 11px; color: #999;">
                                * 3가지 지표(추세/주간/월간) 합산하여 총점 계산<br>
                                * 각 지표는 독립적으로 0~2점 부여 (총점 0~6점)
                            </p>
                        </td>
                    </tr>
...
def aggregate_daily_sales(
    ...
    **context
    ...
    # ⭐ 지점명 변경 매핑 적용 (수집 시 이전 이름 → 현재 이름)
    orders_df['매장명_clean'] = normalize_store_names(orders_df['매장명_clean'])
    
    print(f"[매장명 정제] 완료 (지점명 매핑 적용)")
    ...
    # ⭐ 실오픈일 데이터 타입 정규화 (min() 에러 방지)
    if '실오픈일' in orders_df.columns:
        # 문자열 → 날짜로 변환 시도
        orders_df['실오픈일'] = pd.to_datetime(
            orders_df['실오픈일'], 
            errors='coerce',  # 변환 실패 시 NaT
            format='mixed'    # 다양한 날짜 형식 허용
        )
        nat_count = orders_df['실오픈일'].isna().sum()
        print(f"[실오픈일] 변환 완료 - NaT: {nat_count}건 / 전체: {len(orders_df)}건")
    ...
    daily_agg['order_daily'] = parse_mixed_date(daily_agg['order_daily'])
    ...
    for metric in metrics_to_pivot:
        ...
        # ⭐ Excel 직렬값 처리를 포함한 날짜 변환 (merge 시 타입 일치)
        pivot['order_daily'] = parse_mixed_date(pivot['order_daily'])
...
def calculate_scores(
    ...
    **context
    ...
    try:
        ...
        # 당월 누적합 → amt_curr_mtd
        df['amt_curr_mtd'] = df.groupby(
            ['매장명', '_order_month']
        )['total_amount'].cumsum().astype(int)

        # 전월 동일 일자 누적합 (lookup 방식, 임시 컨럼)
        _mtd_lookup = df[['매장명', '_order_month', '_order_day', 'amt_curr_mtd']].copy()
        _mtd_lookup['_join_month'] = _mtd_lookup['_order_month'] + 1
        _mtd_lookup = _mtd_lookup.rename(columns={'amt_curr_mtd': 'amt_prev_mtd'})
        df = df.merge(
            _mtd_lookup[['매장명', '_join_month', '_order_day', 'amt_prev_mtd']],
            left_on=['매장명', '_order_month', '_order_day'],
            right_on=['매장명', '_join_month', '_order_day'],
            how='left'
        ).drop(columns=['_join_month'])
        df['amt_prev_mtd'] = df['amt_prev_mtd'].fillna(0).astype(int)
        df.drop(columns=['_order_month', '_order_day'], inplace=True)
        print(f"[월간 누적합] amt_curr_mtd/amt_prev_mtd 계산 완료")
        ...
        df_prev = df[['order_daily', '매장명', 'total_amount']].copy()
        ...
        df = df.merge(
            monthly_total[['매장명', '담당자', '년월_next', '전월_매출']],
            left_on=['매장명', '담당자', '년월'],
        ...
        column_order = [
            ...
            'amt_curr_mtd',
            'sum_7d_recent', 'sum_7d_prev',
            'score_trend', 'score_7d_total', 'score_4week_total', 'score',
            'status', 'pre_status',
            '전일_매출', '전일대비_증감률',
            '전주_매출', '전주_주문건수', '전주대비_증감률',
            '전월_매출', '전월대비_증감률',
...
def filter_alerts(
    ...
    **context
    ...
    # 시스템 금일(order_daily) 데이터는 제외하고 처리
    today_dt = pd.Timestamp(datetime.now().date())
    before_today_filter = len(df)
    df = df[df['order_daily'].notna()].copy()
    df = df[df['order_daily'].dt.normalize() != today_dt].copy()
    removed_today = before_today_filter - len(df)
    print(f"[금일 제외] 기준일: {today_dt.strftime('%Y-%m-%d')} / 제외 건수: {removed_today:,}건")

    if df.empty:
        print("[경고] 금일 데이터 제외 후 처리할 데이터가 없습니다 → 알림 대상 없음")
        ti.xcom_push(key=output_xcom_key, value=None)
        return "알림 대상 없음 (금일 데이터 제외 후 0건)"
    ...
    try:
        ...
        # 불필요한 컬럼 제거
        cols_to_drop = ['_row_hash', 'collected_at']
    ...
    # ============================================================
    # 4. Parquet 저장 (XCom 전달용) - ⭐ 전체 데이터 전달 (SMD_07에서 다시 필터링)
    # ============================================================
    temp_dir = LOCAL_DB / 'temp'
    temp_dir.mkdir(exist_ok=True, parents=True)
    ...
    print(f"[Parquet] 저장: {parquet_output.name} (전체 {len(combined_df):,}건)")
